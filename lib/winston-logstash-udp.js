...
LogstashUDP.prototype.log = function(level, msg, meta, callback) {
    ...
    // There is no need to wait for a callback as there is no guarantee that
    // message will ever reach destination and we never get the feedback with UDP.
    // Having self.emit('logged', true) as part of callback in sendLog would
    // cause it to take 10% more time.
